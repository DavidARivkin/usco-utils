...
function extractChanges(prev, cur) {
  ...
  if (delta && '_t' in delta) {
    ...
    result.added = (0, _utils.toArray)(added).filter(i => i !== undefined);
    result.removed = (0, _utils.toArray)(removed).filter(i => i !== undefined);
...
function extractChangesBetweenArrays(prev, cur) {
  ...
  if (delta && '_t' in delta) {
    ...
    result.added = (0, _utils.toArray)(added).filter(i => i !== undefined);
    result.removed = (0, _utils.toArray)(removed).filter(i => i !== undefined);
    result.upserted = (0, _utils.toArray)(upserted).filter(i => i !== undefined);
    // console.log("added",result.added)
    // console.log("removed",result.removed)
  } else if (prev === undefined) {
      // not handled right in the above case for some reason ??
